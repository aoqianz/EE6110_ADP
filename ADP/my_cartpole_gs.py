...
class CartPoleGS(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    def step(self, action):
        ...
        self.x_integral += -x
        self.th_integral += -theta

        force = self.pid(action, -x, -x_dot, -theta, -theta_dot)
        force = force * (1+self.make_actuator_noise())
        force = float(max(min(self.force_mag, force), -self.force_mag))
    ...
    def pid(self, gains, x, xdot, th, thdot):
        force1 = gains[0] * x + gains[1] * self.x_integral + gains[2] * xdot
        force2 = gains[3] * th + gains[4] * self.th_integral + gains[5] * thdot
