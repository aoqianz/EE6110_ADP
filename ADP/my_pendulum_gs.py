...
class PendulumGS(gym.Env):
    ...
    def step(self, gains):
        ...
        theta_noise = float(th*(1+self.make_sensor_noise()))
        thetadot_noise = float(thdot*(1+self.make_sensor_noise()))

        g = self.g
        m = self.m
        l = self.l
        dt = self.dt
        self.integral += -theta_noise
        u = self.pid(theta_noise, thetadot_noise, gains)
        ...
        if self.render_mode == "human":
            self.render()
        return np.array([theta_noise, thetadot_noise], dtype=np.float32), -costs, False, truncated, {}

    def pid(self, th, thdot, gains):
        pid = gains[0] * -th + gains[1] * self.integral + gains[2] * -thdot
        return pid
